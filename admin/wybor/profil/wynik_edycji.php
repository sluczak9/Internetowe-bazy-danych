...
if (strlen($_POST['haslo']) < 6 && strlen($_POST['haslo2'])<6)
{
  ...
  echo "<p class=\"mt-5\"><a class=\"btn btn-primary\" href=\"?wybor=edycja_profilu\">Powrót</a></p>\n";
}
else if ($_POST['haslo'] == $_POST['haslo2'] && $licz == 0)
{
    $hashedpassword = password_hash($_POST['haslo'], PASSWORD_DEFAULT);
    $query_update = "UPDATE `".$prefix."_admin`  SET `haslo` = '" . $hashedpassword . "',`imie`= '" . $_POST['imie'] . "',
    `nazwisko`= '" . $_POST['nazwisko'] . "' WHERE `login`='".$login."'";
